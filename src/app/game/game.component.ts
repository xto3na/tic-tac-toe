...
export class GameComponent implements OnInit {
  ...
  lastMovesComp:Cell[] = new Array<Cell>();
  lastMovesPlayer:Cell[] = new Array<Cell>();
  ...
  move(i:number, j:number) {
    ...
    this.lastMovesPlayer.push(this.cells[i][j]);
    if(this.isWonPlayer()) {
  ...
  moveComputer() {
    if(this.cells[2][2].enabled) {
      ...
      let targetLine = this.getPriorityLine();
      if(this.checkLine(targetLine)) {
        for(let i=0;i<5;i++) {
          if(targetLine.lineCells[i].enabled) {
            this.setSymbol(targetLine.lineCells[i].xCoord, targetLine.lineCells[i].yCoord, this.compSymbol);
  ...
  checkLine(line:Line) {
    ...
    for(let i=0; i<5; i++) {
      if(!line.lineCells[i].enabled && line.lineCells[i].symbol === this.playerSymbol) {
        line.isEnable = false;
        // if(this.lines[this.priorityLineIndex] === line) {
        //   this.priorityLineIndex++;
        //   if(this.priorityLineIndex == 12) {
        //     this.priorityLineIndex = 0;
        //   }
        // }
        return false;
      }
      if(!line.lineCells[i].enabled && line.lineCells[i].symbol === this.compSymbol) {
        countPoints++;
      }
    }
    line.priority = countPoints;
    if(line.priority > this.lines[this.priorityLineIndex].priority) {
      this.priorityLineIndex = line.id;
    }
    return true;
  }
  ...
  getPriorityLine() {
    ...
    // return this.lines[this.priorityLineIndex];
  }
